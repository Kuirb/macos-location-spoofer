/*
 * macOS/Shadowrocket adaptation modified in 2026.
 * SPDX-License-Identifier: AGPL-3.0-only
 * See NOTICE for upstream attribution and modification details.
 *
...
(function () {
  ...
  function parseArpc(bytes) {
    ...
    return {
      ...
      suffix: bytes.slice(state.offset + payloadLength)
    };
  }
  ...
  function spoofAppleResponse(responseBytes, configInput) {
    ...
    if (extraction.kind === "arpc") {
      ...
      response = concatBytes([
        buildAppleWLocResponse(patched.payload, extraction.prefix),
        extraction.suffix || bytesFromArray([])
      ]);
    } else {
      // A bare protobuf payload must remain bare.
      response = patched.payload;
    }
  ...
  function readScriptArguments() {
    ...
    // Module arguments are the only implicit configuration source. In
    // particular, do not read generic keys from Shadowrocket's shared
    // persistent store: stale values from another script could otherwise
    // override the generated plain-text configuration or trigger a request.
    return out;
  }
  ...
  function loadRuntimeConfigSync() {
    var args = readScriptArguments();
    var argumentConfig = configFromArgs(args);
    var cfg = mergeConfig(DEFAULT_CONFIG, argumentConfig);
    ...
    // Explicit module arguments are authoritative over optional address or
    // remote configuration sources.
    cfg = mergeConfig(cfg, argumentConfig);
    ...
    for (key in sourceHeaders) {
      if (Object.prototype.hasOwnProperty.call(sourceHeaders, key)) {
        ...
        if (
          ...
          lower.indexOf("x-location-spoofer-") !== 0
        ) {
  ...
  function continueResponseRewrite(config, preparedResponseBody) {
    var responseBody = bodyToBytes(preparedResponseBody) || messageBodyToBytes($response);
  ...
  function prepareResponseBody(config) {
    return prepareResponseBodySync(config);
  }
    ...
    loadRuntimeConfig(function (config) {
      ...
      try {
        ...
        if (hasResponse) {
          ...
          var preparedResponseBody = prepareResponseBody(config);
          continueResponseRewrite(config, preparedResponseBody);
          return;
        }
